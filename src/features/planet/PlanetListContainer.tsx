...
const ModalView = ({actions, planetId}: Props) => {
  ...
  return <div>
    ...
    <button className={"btn btn-warning"} onClick={actions.handleCloseButtonClick}>{"cancel"}</button>
  </div>;
}
...
export const PlanetListContainer = () => {
  ...
  const showPlanetListView = () => hasValue(planets.length) ?
  ...
  return showConfirmDeleteModal ?
    <Modal bodyContent={<ModalView actions={{handleCloseButtonClick: handleCloseModalEvent}} planetId={localPlanetId}/>}
           onModalClose={handleCloseModalEvent}
           modalSize={"modal-sm"}
           title={"Delete planet?"}/> : showPlanetListView()
};
